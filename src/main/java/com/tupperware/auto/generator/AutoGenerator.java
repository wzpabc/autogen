			...
			try {
				...
				if (osName != null) {
					if (osName.contains("Mac")) {
						...
						Runtime.getRuntime().exec("cmd /c start " + config.getGlobalConfig().getOutputDir()+File.separator+"formatjsp.bat "+config.getGlobalConfig().getOutputDir());
	...
	private Map<String, VelocityContext> analyzeData(ConfigBuilder config) {
		...
		for (TableInfo tableInfo : tableList) {
			...
			if (null != cfg) {
				 ...
				 * 注入自定义配置
				 */
				cfg.initMap(tableInfo.getName());
				ctx.put("cfg", cfg.getController());
			}
