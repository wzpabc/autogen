 ...
 * the License.
 */
package com.tupperware.auto.generator.config.builder;
...
import com.tupperware.auto.generator.config.GlobalConfig;
import com.tupperware.auto.generator.config.PackageConfig;
import com.tupperware.auto.generator.config.TemplateConfig;
import com.tupperware.auto.generator.config.po.TableField;
import com.tupperware.auto.generator.config.po.TableInfo;
import com.tupperware.auto.generator.config.DataSourceConfig;
...
public class ConfigBuilder {
	...
	public ConfigBuilder(PackageConfig packageConfig, DataSourceConfig dataSourceConfig, StrategyConfig strategyConfig,
		...
		if (null == packageConfig) {
			handlerPackage(this.globalConfig, new PackageConfig());
		} else {
			handlerPackage(this.globalConfig, packageConfig);
		}
	...
	private void handlerPackage(String outputDir, PackageConfig config) {
		...
		packageInfo.put(ConstVal.XML, joinPackage(config.getResource(), config.getXml()));
...
//		packageInfo.put(ConstVal.SQL, joinPackage(config.getParent(), config.getSql()));
...
//		pathInfo.put(ConstVal.SQL_PATH, joinPath(outputDir, packageInfo.get(ConstVal.SQL)));
...
//		pathInfo.put(ConstVal.EDIT_PATH, joinPath(outputDir, "jsp"));
	 ...
	 *            PackageConfig
	 */
	private void handlerPackage(GlobalConfig conf, PackageConfig config) {
		packageInfo = new HashMap<String, String>();
		packageInfo.put(ConstVal.MODULENAME, config.getModuleName());
		packageInfo.put(ConstVal.ENTITY, joinPackage(config.getParent(), config.getEntity()));
		packageInfo.put(ConstVal.MAPPER, joinPackage(config.getParent(), config.getMapper()));
...
//		packageInfo.put(ConstVal.SQL, joinPackage(config.getParent(), config.getSql()));
		...
		pathInfo = new HashMap<String, String>();
		pathInfo.put(ConstVal.ENTITY_PATH, joinPath(conf.getOutputJavaDir(), packageInfo.get(ConstVal.ENTITY)));
		pathInfo.put(ConstVal.MAPPER_PATH, joinPath(conf.getOutputJavaDir(), packageInfo.get(ConstVal.MAPPER)));
		pathInfo.put(ConstVal.XML_PATH, joinPath(conf.getOutputResDir(), packageInfo.get(ConstVal.XML)));
		pathInfo.put(ConstVal.SERIVCE_PATH, joinPath(conf.getOutputJavaDir(), packageInfo.get(ConstVal.SERIVCE)));
		pathInfo.put(ConstVal.SERVICEIMPL_PATH, joinPath(conf.getOutputJavaDir(), packageInfo.get(ConstVal.SERVICEIMPL)));
		pathInfo.put(ConstVal.CONTROLLER_PATH, joinPath(conf.getOutputJavaDir(), packageInfo.get(ConstVal.CONTROLLER)));

		pathInfo.put(ConstVal.FORM_PATH, joinPath(conf.getOutputJavaDir(), packageInfo.get(ConstVal.FORM)));
//		pathInfo.put(ConstVal.SQL_PATH, joinPath(outputDir, packageInfo.get(ConstVal.SQL)));
...
//		pathInfo.put(ConstVal.EDIT_PATH, joinPath(outputDir, "jsp"));
	...
	private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, String tablePrefix) {
		for (TableInfo tableInfo : tableList) {
			...
			if (StringUtils.isNotEmpty(globalConfig.getSqlName())) {
				tableInfo.setSqlName(String.format(globalConfig.getSqlName(), tableInfo.getEntityName()));
			} else {
				tableInfo.setSqlName(tableInfo.getEntityName() + ConstVal.SQL);
			}
			tableInfo.setSqlName(toLowerCaseFirstOne(tableInfo.getSqlName()));
		...
		try {
			...
			while (results.next()) {
				...
				if (StringUtils.isNotEmpty(tableName)) {
					...
					if(querySQL.getDbType().equals("sqlserver")) {					
						String tableSchema=results.getString(querySQL.getTableSchema());
						tableInfo.setSchemaName(tableSchema);
					}
					if (isInclude) {
						if(config.getInclude()[0].equalsIgnoreCase("all")){
					...
					if (StringUtils.isNotEmpty(tableInfo.getName())) {
						List<TableField> fieldList = getListFields(tableInfo.getName(),tableInfo.getSchemaName(), fieldStrategy);
						tableInfo.setFields(fieldList);
//						tableInfo.setName(tableInfo.getSchemaName()+""+tableInfo.getName());
	 ...
	 * @return 表信息
	 */
	private List<TableField> getListFields(String tableName,String schemaName, NamingStrategy strategy) throws SQLException {
		boolean havedId = false;
		PreparedStatement pstate ;
		if(querySQL.getDbType().equals("sqlserver")) {
			pstate= connection.prepareStatement(String.format(querySQL.getTableFieldsSql(), tableName,schemaName));
		}else {			
			pstate= connection.prepareStatement(String.format(querySQL.getTableFieldsSql(), tableName));
		}
		ResultSet results = pstate.executeQuery(); 
